...
class AirSimEnv(gym.Env):
    ...
    def computeReward(self, now):
        ...
        distance_now = np.sqrt(np.power((self.goal.x_val-now.x_val),2) + np.power((self.goal.y_val-now.y_val),2)+ np.power((self.goal.z_val-now.z_val),2))
        ...
        r = r - (distance_now*0.1)#(distance_before - distance_now)
    ...
    def step(self, action):
        ...
        now = airgym.simGetGroundTruthKinematics(self.drone1_vehicle_name).position
        ...
        if collided == True:
            ...
            if re.match(r'Target\d',collided_with):
                ...
                reward = -100.0  
            distance = np.sqrt(np.power((self.goal.x_val-now.x_val),2) + np.power((self.goal.y_val-now.y_val),2)+ np.power((self.goal.z_val-now.z_val),2))
        ...
        # Youuuuu made it
        if distance < self.target_thershold:
            print('Hi hit my target with distance',distance)
            done = True
            reward = 100.0
